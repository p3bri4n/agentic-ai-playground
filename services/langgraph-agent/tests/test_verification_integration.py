...
PLANNER_ENABLED + VERIFICATION_ENABLED activés ensemble) : un scénario
retry-puis-succès et un scénario budget+replan épuisés aboutissant à
report_failure/END. Bornés à ces deux cas plutôt qu'une matrice complète,
pour garder la suite lisible (voir docs/briefs/phase-1-coeur-cognitif.md).
"""
...
async def test_retry_then_success_reaches_fait_and_final_answer(mock_side_services, monkeypatch):
    ...
    route.side_effect = [
        ...
        _sse(tool_call_response("mouse_click", "call_1", '{"x": 1, "y": 1}')),  # call_llm #1
        httpx.Response(200, json=non_streaming_response(json.dumps({"atteint": False, "raison": "pas encore"}))),  # verify #1
        _sse(tool_call_response("mouse_click", "call_2", '{"x": 2, "y": 2}')),  # call_llm #2 (stratégie différente)
        httpx.Response(200, json=non_streaming_response(json.dumps({"atteint": True, "raison": "fait"}))),  # verify #2
        _sse(text_response(["Fait", "."])),  # call_llm #3 : réponse finale
    ]
    ...
    assert last_message.content == "Fait."
...
async def test_budget_and_replan_exhausted_reaches_report_failure(mock_side_services, monkeypatch):
    ...
    route.side_effect = [
        httpx.Response(200, json=non_streaming_response(plan_json)),  # plan_task
        _sse(tool_call_response("mouse_click", "call_1", '{"x": 1, "y": 1}')),  # call_llm #1
        httpx.Response(200, json=non_streaming_response(json.dumps({"atteint": False, "raison": "echec1"}))),  # verify -> echoue (budget=1)
        httpx.Response(200, json=non_streaming_response(replan_json)),  # replan_task
        _sse(tool_call_response("mouse_click", "call_2", '{"x": 9, "y": 9}')),  # call_llm #2
        httpx.Response(200, json=non_streaming_response(json.dumps({"atteint": False, "raison": "echec2"}))),  # verify -> echoue à nouveau
    ]
    g.agent_graph = g.build_graph()
